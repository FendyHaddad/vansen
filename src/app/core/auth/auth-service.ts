...
export class AuthService {
  ...
  /**
   * Local scope only: this device's session, not every device and not the
   * user's connected assistants (Settings → Connected assistants owns those).
   * A global sign-out here would also revoke every MCP grant, forcing Claude
   * or ChatGPT to re-run OAuth the next time they are used.
   */
