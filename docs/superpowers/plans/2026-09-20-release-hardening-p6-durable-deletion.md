...
- [x] **Step 1: Gather what the product currently promises**
...
- [x] **Step 2: Write the spec with the decisions filled in**
...
- [x] **Step 3: Resolve every `<DECIDE:>` with the user**
...
- [x] **Step 1: Add RED fixtures covering each real artifact kind**
...
- [x] **Step 2: Create durable locators and outbox records**
...
- [x] **Step 3: Implement enqueue, claim and completion RPCs**
...
- [x] **Step 4: Make deletion a lifecycle, including account/lapse paths**
...
- [x] **Step 1: Define the correct object API and RED tests**
...
- [x] **Step 2: Implement leased retries and verify disappearance**
...
- [x] **Step 3: Wire worker auth and scheduling**
...
- [x] **Step 1: Write route/lifecycle RED cases**
...
- [x] **Step 2: Remove inline cancellation and direct deletes**
...
- [x] **Step 3: Reconcile every subscription before claiming account closure**
...
- [x] **Step 4: Align settings and policy copy**
...
- [x] **Step 1: Implement paginated inventory of every configured store**
...
- [x] **Step 2: Add retained reconciler tests**
...
- [x] **Step 3: Rehearse and record every deletion path**
...
- [x] **Step 4: Verify the complete plan**
...
- [x] Every object has a verified backend/bucket/path, including uploads, ZIPs, posters and quarantine.
- [x] Every delete path hides content, safely settles/reconciles pending work, and durably schedules cleanup before dropping ownership.
- [x] A failed backend or stale lease never reports completed removal; repeated deletion is idempotent.
- [x] Financial/expense history and approved evidence retention survive account closure as specified by D2.
- [x] Provider LoRA deletion and subscription limitations are explicit and tracked.
- [x] Full inventory covers Supabase and R2 with all pages; incomplete access fails verification.
- [x] D2 policy, settings/privacy/help and vansen.md state the same lifecycle and durations.
- [x] Worker remains undeployed until P9; local proof is not production proof.

## Execution notes (2026-09-21)

Verification log: `docs/superpowers/plans/2026-09-20-deletion-verification-log.md`.
Policy: `docs/superpowers/specs/2026-09-20-retention-policy.md` (D2).

Two decisions were taken during execution and are recorded here because they
change the plan's shape:

1. **A provider artifact blocks completion, not finalisation.** The plan's
   constraint ("never 'completed' with unresolved provider work") would, taken
   at the finalisation step, have kept a customer's own data alive while we
   waited on a third party with no deletion API at all. The closure therefore
   finalises (profile, content, objects, anonymisation) and stops at
   `processing` with `providerArtifacts > 0` until the artifact is `confirmed`
   or recorded `unsupported`. `provider_artifact_deletions.deletion_ref` keeps
   the handle after the owner is anonymised.
2. **Nothing resolves a provider artifact yet.** The capability record
   documents no artifact-deletion API for any provider, so no closure with a
   trained persona can reach `completed`. This is visible and honest, not
   silent — but it needs an owner decision before release (open item 3 in the
   verification log).

Also changed beyond the written steps: `dropLostObject` in
`_shared/jobs/store.ts` now queues a losing attempt's object through the
registry instead of deleting it best-effort, and the lapse countdown in the UI
(`daysUntilPurge`) replaces `graceDaysLeft`, which counted a 30-day window
after the period ended that D2 removes.
